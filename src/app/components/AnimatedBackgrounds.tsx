...
export function FallingStarsBg({ opacity = 0.3 }) {
  ...
  const createStarElement = (key: number) => {
    ...
    return (
      <div 
        key={key}
        className={`absolute w-${size} h-${size} ${starColor} ${animationsEnabled ? 'animate-falling-star animate-pulse-glow' : ''} pointer-events-none`}
        style={{
          left: leftPosition,
          top: `-20px`,
          width: `${size}px`,
      ...
      >
        ...
        >
          <path d="M10,2 h4 v2 h2 v2 h2 v4 h-2 v2 h-2 v2 h-4 v-2h-2 v-2h-2 v-4h2 v-2h2 z" />
        </svg>
      </div>
    )
  }
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className={`absolute top-[15%] right-[35%] w-8 h-8 bg-yellow-400/20 rounded-full blur-sm transition-all duration-5000 ease-in-out ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`}></div>
        <div className={`absolute bottom-[25%] left-[40%] w-12 h-12 bg-white/10 rounded-lg rotate-45 blur-sm transition-all duration-7000 ease-in-out ${isLoaded && animationsEnabled ? 'animate-float' : ''}`}></div>
      </div>
...
export function ProjectsBackground() {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        {/* Soft color accents */}
        <div className={`absolute top-[10%] right-[10%] w-72 h-72 bg-gradient-to-r from-accent-primary/10 to-accent-secondary/10 rounded-full blur-3xl ${animationsEnabled ? 'animate-pulse-glow' : ''}`}></div>
        <div className={`absolute bottom-[5%] left-[5%] w-96 h-96 bg-gradient-to-r from-accent-secondary/10 to-accent-primary/10 rounded-full blur-3xl ${animationsEnabled ? 'animate-pulse-glow' : ''}`} style={{ animationDelay: '2s' }}></div>
        ...
        <div className={`absolute top-[45%] left-[25%] w-6 h-6 bg-accent-primary/15 rounded-full blur-sm ${isLoaded && animationsEnabled ? 'animate-float' : ''}`} style={{ animationDuration: '4s' }}></div>
        <div className={`absolute bottom-[35%] right-[30%] w-10 h-10 bg-accent-secondary/15 rounded-lg rotate-12 blur-sm ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`} style={{ animationDuration: '6s' }}></div>
...
export function SkillsBackground() {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        {/* Circular glows */}
        <div className={`absolute -top-[10%] left-[20%] w-96 h-96 bg-yellow-400/15 rounded-full blur-3xl ${animationsEnabled ? 'animate-pulse-glow' : ''}`} style={{ animationDuration: '8s' }}></div>
        <div className={`absolute bottom-[0%] right-[10%] w-80 h-80 bg-yellow-300/15 rounded-full blur-3xl ${animationsEnabled ? 'animate-pulse-glow' : ''}`} style={{ animationDuration: '10s', animationDelay: '1s' }}></div>
        
        {/* Animated floating squares */}
        <div className={`absolute top-[20%] right-[25%] w-8 h-8 bg-yellow-400/15 rotate-45 blur-sm ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`}></div>
        <div className={`absolute bottom-[30%] left-[15%] w-12 h-12 bg-yellow-400/15 rotate-12 blur-md ${isLoaded && animationsEnabled ? 'animate-float' : ''}`} style={{ animationDelay: '1s' }}></div>
        <div className={`absolute top-[60%] left-[40%] w-6 h-6 bg-yellow-300/15 rotate-45 blur-sm ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`} style={{ animationDelay: '0.5s' }}></div>
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Animated floating elements */}
        <div className={`absolute top-[30%] left-[40%] w-8 h-8 bg-accent-primary/15 rounded-full blur-sm ${isLoaded && animationsEnabled ? 'animate-float' : ''}`} style={{ animationDuration: '5s' }}></div>
        <div className={`absolute bottom-[25%] right-[35%] w-10 h-10 bg-accent-secondary/15 rounded-lg rotate-12 blur-sm ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`} style={{ animationDuration: '7s' }}></div>
      </div>
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className={`absolute top-[30%] right-[20%] w-12 h-12 bg-white/5 rounded-lg rotate-45 blur-md ${isLoaded && animationsEnabled ? 'animate-float-slow' : ''}`}></div>
        <div className={`absolute bottom-[20%] left-[25%] w-8 h-8 bg-accent-primary/10 rounded-full blur-sm ${isLoaded && animationsEnabled ? 'animate-float' : ''}`} style={{ animationDelay: '1s' }}></div>
